...
bool Shape::checkWalls(const sf::Vector2u & window_size)
{
    ...
    // if hit the wall, make new shape turns true, so we have to create a new shape
    //make_new_shape = hit_a_wall;
...
Shape * Shape::getNewShape(){
    ...
    // if doens't hit the wall, it returns null, so there is not new shape
    if (! make_new_shape)
        return NULL;
...
bool Shape::shouldBeDeleted(){


    // if returs true, it has to be delete
